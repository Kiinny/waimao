...
+      { userId: "ops-1", roles: ["OPERATIONS"], permissions: ["report.read"] },
+      { type: "after-sales" },
+    );
+
...
+    const sales = { userId: "sales-1", roles: ["SALES_REP"], permissions: ["report.read"] };
+    const operations = { userId: "ops-1", roles: ["OPERATIONS"], permissions: ["report.read"] };
...
+
+const ROLE_REPORT_TYPES: Record<string, readonly ReportType[]> = {
+  SALES_REP: ["sales", "collections", "receivables", "customers", "markets", "products", "representatives"],
+  SALES_MANAGER: ["sales", "collections", "receivables", "customers", "markets", "products", "representatives", "conversion"],
+  FINANCE: ["sales", "collections", "receivables", "customers", "markets", "products", "representatives", "profit"],
+  PROCUREMENT: ["products", "suppliers", "purchasing"],
+  OPERATIONS: ["logistics", "after-sales"],
+};
+
...
+    (context.roles ?? []).flatMap((role) => ROLE_REPORT_TYPES[role] ?? []),
+  );
...
+  }
+  $artifactPath = [System.IO.Path]::GetFullPath((Join-Path $source ([string]$file.path)))
