...
index a09619c..562d160 100644
--- a/prisma/seed.ts
+++ b/prisma/seed.ts
@@ -886,19 +886,245 @@ async function main() {
...
+                  inventoryItemId: inventory.id,
+                  serialNumber: transaction.inventorySerial?.serialNumber ?? null,
...
+            failureMessage={locale === "zh" ? "无法创建供应商。" : "Could not create supplier."}
+            loadingLabel={locale === "zh" ? "创建中…" : "Creating…"}
+            submitLabel={locale === "zh" ? "创建供应商" : "Create supplier"}
+            successMessage={locale === "zh" ? "供应商已创建。" : "Supplier created."}
+          >
+            <label>{locale === "zh" ? "编号" : "Code"}<input name="code" required /></label>
...
+    : 0;
+  const categories = [
+    ...new Set(supplier.products.map(({ product }) => product.categoryId)),
+  ];
+  const brands = [
+    ...new Set(
+      supplier.products
...
+              <span>{product.brand ?? (locale === "zh" ? "无品牌" : "Unbranded")}</span>
...
+            <SupplierEditForm locale={locale} supplier={supplier} />
+          </details>
+          <SupplierArchiveAction
+            expectedVersion={supplier.version}
+            locale={locale}
...
+    const context = await currentAuthorizationContext();
+    requirePermission(context, "supplier.read");
+    const row = await service.getSupplier((await params).id);
+    if (!row) {
+      throw new DomainError("SUPPLIER_NOT_FOUND", "Supplier not found", 404);
+    }
+    return success(row);
...
+    const context = await currentAuthorizationContext();
+    requirePermission(context, "purchase.read");
+    const row = await service.getPurchaseOrder((await params).id);
+    if (!row) {
+      throw new DomainError(
+        "PURCHASE_ORDER_NOT_FOUND",
+        "Purchase order not found",
