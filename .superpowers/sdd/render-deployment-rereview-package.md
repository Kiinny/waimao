# Render deployment rereview package

## Deployment files

diff --git a/render.yaml b/render.yaml
new file mode 100644
index 0000000..e782bb5
--- /dev/null
+++ b/render.yaml
@@ -0,0 +1,32 @@
+services:
...
new file mode 100644
index 0000000..a094b7b
--- /dev/null
+++ b/.env.example
@@ -0,0 +1,17 @@
+# Local development examples only. Supply production values through Render; never commit secrets.
+# On Render, DATABASE_URL is Neon's pooled TLS URL, DIRECT_URL is its direct TLS URL,
+# and AUTH_URL is the exact public Render HTTPS URL. Prisma CLI falls back to DATABASE_URL locally.
+DATABASE_URL=postgresql://crm:crm_dev_password@localhost:5432/foreign_trade_crm?schema=public
+DIRECT_URL=postgresql://crm:crm_dev_password@localhost:5432/foreign_trade_crm?schema=public
...
+   ```
+
+   This process-scoped value is removed even if the seed fails. Do not paste the URL into a tracked file.
...
+
 ...
 pnpm prisma:migrate    # create/apply development migrations
 pnpm prisma:seed       # deterministic roles, users and foundation data
@@ -69,18 +111,74 @@ pnpm prisma:seed       # deterministic roles, users and foundation data
